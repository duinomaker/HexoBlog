...
若 $n$ 为质数，且 $1\leq a\leq n-1$，那么根据费马小定理，可知 $a^{n-1}-1\equiv0\\;({\rm mod}\\;n)$，那么根据上式，可以得出
...
可以证明 <sup class="cite">[<a href="#cite-1">1</a>]</sup>，若一奇数是合数，那么 $\lbrace2,\ldots,n-2\rbrace$ 中超过 $75\\%$ 的数都是 $Miller\text{-}\\!Rabin\\;witness$。所以经过 $k$ 轮的随机检测，$Miller\text{-}\\!Rabin$ 判断某个数是质数时，它有概率至少为 $(1-4^{-k})$ 的把握。
