...
简单路径即不重复经过同一点的路径，这题考察了深度优先搜索技巧。记录下某一点所指向的点$(to_i)$，该点是否已被访问过$(vis_i)$，以及以该点为起点最多经过多少点$(len_i)$&hairsp;。
...
清点前需记录下覆盖范围的端点$(node)$，包括其坐标$(pos)$&hairsp;、左端点还是右端点$(lr)$&hairsp;、${\rm A}$ 类云还是 ${\rm B}$ 类云$(type)$&hairsp;。
