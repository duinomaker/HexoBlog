...
$1\leq K\leq N\leq 100,000$ 输入的所有数字不超过 $10^8$
...
简单路径即不重复经过同一点的路径，这题考察了深度优先搜索技巧。记录下某一点所指向的点$(to_i)$，该点是否已被访问过$(vis_i)$，以及以该点为起点最多经过多少点$(len_i)$ 。
...
对于所有的数据 $1\leq N\leq 100,000$ 且 $1\leq X_i,Y_i\leq N$ ，$X_i$ 互不相等且 $Y_i$ 互不相等
...
$1\leq N\leq 100,000$ 且 $0\leq A_i,B_i\leq 10^9$
...
输入包含 $T$ 组数据，$1\leq T\leq 10,000$ 且 $1\leq N\leq 10^{18}$
...
$1\leq H\leq 500,000$ 且 $1\leq x_i,y_i,z_i,N,M\leq 2000$ 且 $1\leq x_i\leq N$ 且 $1\leq y_i\leq M$
...
加权求和。每个方块的权值是已知的$(i+j)$，一个点对总伤害的贡献，等于该行和该列所有权值的和，乘以伤害。
...
1. 空序列是合法括号序列
2. 如果 $S$ 是一个合法括号序列,那么 ${\rm (S)}$ 是合法括号序列
3. 如果 $A$ 和 $B$ 都是合法括号序列,那么 ${\rm AB}$ 是一个合法括号序列

现在给定一个括号序列，求最少删去几个括号能得到一个合法的括号序列
输入包含 $T$ 组数据，每组数据中，设括号序列的长度为 $N$
$1\leq T,\sum^T{(N)}\leq 1,000,000$
...
$1\leq N,M\leq 100,000$ 且 $1\leq |X_i|,|Y_i|,|P_i|,|Q_i|\leq 10^9$
...
一朵云的位置用一行四个数 $A_i,B_i,C_i,D_i$ 表示，意为这朵云左上角在 $(A_i,B_i)$,右下角在$(C_i,D_i)$
...
但如果我们在坐标系中添加这样一条辅助线$(x+y=0)$呢？

<img class="lazyload" src="/images/nowcoder-2020-winter-camp-6_1.jpg" width="100%">
...
所以可以采用投影的方法，将云彩投影到辅助线上，得到一条线段，其在辅助线上的覆盖范围是 $[a-b,c-d]$ 。最后清点有重合部分的线段对数，即可得到答案。

清点前需记录下覆盖范围的端点$(node)$，包括其坐标$(pos)$、左端点还是右端点$(lr)$、${\rm A}$ 类云还是 ${\rm B}$ 类云$(type)$ 。
...
对于全部的数据，$1\leq N\leq 500$，输入的所有数字都是不超过 $10^9$ 的非负整数。
