...
简单路径即不重复经过同一点的路径，这题考察了深度优先搜索技巧。记录下某一点所指向的点$(to_i)$，该点是否已被访问过$(vis_i)$，以及以该点为起点最多经过多少点$(len_i)$<span class="fix-ml">。</span>
...
所以可以采用投影的方法，将云彩投影到辅助线上，得到一条线段，其在辅助线上的覆盖范围是 $[a-b,c-d]$<span class="fix-ml">。</span>最后清点有重合部分的线段对数，即可得到答案。

清点前需记录下覆盖范围的端点$(node)$，包括其坐标$(pos)$、左端点还是右端点$(lr)$、${\rm A}$ 类云还是 ${\rm B}$ 类云$(type)$<span class="fix-ml">。</span>
