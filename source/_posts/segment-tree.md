...
因为整棵树的深度是 $\lceil\log N\rceil$，所以单次修改的时间复杂度为 $O(\log N)$<span class="fix-ml">。</span>
...
按顺序执行这 $C$ 条指令，在每条执行后，输出起重机末端(第 $N$ 条线段的端点)的坐标。假设起重机支点的坐标是 $(0,0)$<span class="fix-ml">。</span>
