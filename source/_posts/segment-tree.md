...
<img class="lazyload" src="/images/segment-tree_1.jpg" width="100%" alt="区间长度为 $2$ 的整数次幂">

如图，这就是一棵线段树。我们可以发现，当整个区间统计长度为 $2$ 的整数次幂时，整棵线段树一定是一棵完全二叉树，那我们就可以用堆的编号方法来给线段树来编号啊（其实图中已经编好了）。

如果根节点编号为 $1$ 。编号为 $x$ 的节点，它的左儿子编号为 $2x$，右儿子编号为 $2x+1$ 。

这样，我们就可以用一个数组来存所有节点的编号了！

诶等等，那万一整个区间长度不是 $2$ 的整数次幂呢？

<img class="lazyload" src="/images/segment-tree_2.jpg" width="100%" alt="区间长度不为 $2$ 的整数次幂">
...
$$
\begin{eqnarray}
N=10^4 \rightarrow T=2^{15} \\\\
N=10^5 \rightarrow T=2^{18} \\\\
N=10^6 \rightarrow T=2^{21}
\end{eqnarray}
$$
...
因为整棵树的深度是 $\lceil \log N \rceil$，所以单次修改的时间复杂度为 $O(\log N)$ 。
...
当然，为了维护区间从其端点开始的最大子段和，还需维护区间和$(sum)$ 。如此，一个区间从其左端开始的最大子段和，假设左右儿子的位置分别为 $chl$ 和 $chr$，便可以如此维护：
...
所谓 “合并” 的操作其实就是建树时，子节点向上传播信息的操作，我们可以复用这段代码，也就是下方参考代码中的 ${\rm pushup}$ 函数。
...
有一台起重机。起重机可以看成由 $N$ 条线段首尾相接而成。第 $i$ 条线段的长度是 $L_i$ 。
最开始，所有的线段都笔直连接，指向上方。
...
按顺序执行这 $C$ 条指令，在每条执行后，输出起重机末端(第 $N$ 条线段的端点)的坐标。假设起重机支点的坐标是 $(0,0)$ 。
...
$$
\begin{eqnarray}
x_p & = & x_{chl} & + & (\cos(ang_p) & \times & x_{chr} & - & \sin(ang_p) & \times & y_{chr}) \\\\
y_p & = & y_{chl} & + & (\sin(ang_p) & \times & x_{chr} & + & \cos(ang_p) & \times & y_{chr})
\end{eqnarray}
$$

比较难维护的是第二个值，旋转角度。我随手画了个图方便理解。

<img class="lazyload" src="/images/segment-tree_3.jpg" width="100%">
